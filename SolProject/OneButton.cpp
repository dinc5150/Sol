...
OneButton::OneButton(int pin, int activeLow, Trigger *MasterTrigger)
{
	...
	_clickTicks = 150;        // number of millisec that have to pass by before a click is detected.
...
void OneButton::tick(void)
{
	...
	if (_state == 0) { // waiting for menu pin being pressed.
		...
		if ((buttonLevel == _buttonReleased) && (now < _startTime + _debounceTicks)) {
			...
			_masterTrigger->CallEvent(Trigger::Evt_LongPressStart);
			_masterTrigger->CallEvent(Trigger::Evt_LongPressing);
		...
		if (now > _startTime + _clickTicks) {
			...
			_masterTrigger->CallEvent(Trigger::Evt_Click);
		...
		if (buttonLevel == _buttonReleased) {
			...
			_masterTrigger->CallEvent(Trigger::Evt_DoubleClick);
		...
		if (buttonLevel == _buttonReleased) {
			_isLongPressed = false;  // Keep track of long press state
			_masterTrigger->CallEvent(Trigger::Evt_LongPressStop);
